...
NOT_VALID = ['_', '-', '--', '', '.', 'n/a', 'none', '/', 'na', 'n.a.', 'unknown', 'yes', '✓', 'true', '√', 'no', '✘', 'false', '×'] # no boolean vals

def convert_to_float(value):
    v, fl_v = value, np.nan
    if isinstance(value, str):
        v = v.strip()
        if value.lower() in NOT_VALID:
            return np.nan
        v = v.replace('..', '.') # typos
...
if __name__ == '__main__':
    ...
    MIN_RESULTS, MIN_PROPS, TOPK = 10, 2, 20

    # filter for min evals, properties, and incompleteness
    stats = stats.groupby('eval_id').last()
    filtered = stats[(stats['n_results'] >= MIN_RESULTS) & (stats['n_metrics'] >= MIN_PROPS)]
    ...
    not_mapped = sparse_metrics_stacked[sparse_metrics_proc.isna()]
    # check for amount of successful conversions
    print(f'Could not convert {not_mapped.size / sparse_metrics_proc.size * 100:.2f}% of PWC metric values to positive floats ({pd.unique(not_mapped).size} / {pd.unique(sparse_metrics_proc).size} in total).')
    print('The following metric values could not be transformed:', pd.unique(not_mapped).to_dense())
    ...
    sparse_new = pd.concat([sparse[meta_cols], to_sparse(sparse_metrics_proc.unstack())], axis=1)
    del not_mapped, sparse_metrics_stacked, sparse_metrics_proc, sparse
    ...
    for (task, ds) in pd.unique(sparse_new[['task_STREP$', 'dataset_STREP$']].to_records(index=False)):
        ...
        for col in sub_sparse.select_dtypes('number').columns:
            ...
            if not (values.size > 0 and pd.unique(values).size > 2):
                sub_sparse = sub_sparse.drop(col, axis=1)
            else:
                # Calculate skewness of original and log-transformed values, use log if more appropriate TODO this can surely be improved
                ...
                # Identify outliers on the original or log-transformed scale
                to_drop = ~(sub_sparse[col].between(c_lower_bound, c_upper_bound) | sub_sparse[col].isna())
                if to_drop.sum() > 0:
                    print(f'    {col:<30} - Q: {c_Q1:15.1f} {c_Q3:15.1f} - drop {to_drop.sum():<2} / {values.size:<3} vals: {str(np.unique(sub_sparse[col][to_drop].values))}')
        ...
        # remove nan rows and cols and do another check for min number of model results and properties
        sub_sparse = sub_sparse.dropna(how="all", axis=0).dropna(how="all", axis=1)
        if sub_sparse.shape[0] >= MIN_RESULTS and sub_sparse.select_dtypes('number').shape[1] >= MIN_PROPS:
            ok_data_per_task_and_ds.append(sub_sparse)
        else:
            print(f"Discarding evaluations for {task} on {ds} - numeric properties: {str(sub_sparse.select_dtypes('number').shape)}")

    # merge ok evaluations
    sparse_new = pd.concat(ok_data_per_task_and_ds, axis=0).dropna(how='all', axis=1).reset_index(drop=True)
    ...
    print(':::::::::::::::: PURGED UNINFORMATIVE AND SPARSELY POPULATED EVALUATIONS FROM FULL DB - NEW SHAPE:', sparse_new.shape)
    ...
    res_keys = ['param', 'size', 'flops', 'latenc', 'operation', 'emiss']
    qual_keys = ["sentiment"] # make sure that some properties are not accidently understood as resources
    client = PapersWithCodeClient()
    ...
    missed_metrics = set()
    for eval_id in tqdm(filtered.index, 'assessing property meta information'):
        ...
        for metr in metrics.results:
            if metr.name in sparse_new.columns and metr.name not in meta_cols:
                ...
                group = 'Resources' if (any([k in unified for k in res_keys]) and not any([k in unified for k in qual_keys])) else 'Quality'
                ...
                if m_name in properties: # multiple values found => store the is_loss info (might be contradictory!)
                    ...
                    properties[m_name] =  {
                        "name": m_name, "shortname": unified[:5], "unit": "number",
                        "group": group, "weight": 1, "maximize": not metr.is_loss
                    }
                    if metr.description and len(metr.description) > 0:
                        properties[m_name]["description"] = metr.description
            else:
