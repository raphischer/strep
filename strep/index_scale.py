...
def _compound(input, properties_meta, boundaries):
    results = {}
    results['compound_index'] = _compound_single(input, properties_meta)
    results['resource_index'] = _compound_single(input, {p: v for p, v in properties_meta.items() if v['group'] != 'Performance'})
    results['quality_index'] = _compound_single(input, {p: v for p, v in properties_meta.items() if v['group'] == 'Performance'})
    results = pd.DataFrame(results, index=input.index)
    boundaries = prepare_boundaries(results, boundaries) # either provided from outside, or calculated based on quantiles
    rated = _rate(results, boundaries=boundaries).rename(columns={col: col.replace('index', 'rating') for col in results.columns})
    return pd.concat([results, rated], axis=1)
...
def _scale_single(input, scale_m, meta, reference, mode):
    ...
    if mode == 'rating':
        ...
        reference_local = prepare_boundaries(input[sub_meta.keys()], reference) # reference now controls the rating boundaries per property
    if mode == 'compound':
        assert not np.any(np.logical_or(input[sub_meta.keys()] > 1, input[sub_meta.keys()] < 0)), f'Found values outside of the interval (0, 1] - please properly index-scale your results first!'
    return scale_m(input, sub_meta, reference_local), sub_meta.keys()
...
def scale(input, meta=None, reference=None, mode='index', verbose=True):
    ...
    if len(split_by) > 0:
        ...
        for sub_config, sub_input in input.groupby(split_by): 
            sub_results.append( _scale_single(sub_input, scale_m, meta, reference, mode) )
            if verbose:
                print('   - scaled results for', sub_config)
        sub_results, scaled_cols = zip(*sub_results)
        scaled_cols = list(set([col for sub_cols in scaled_cols for col in sub_cols]))
        ...
        results, scaled_cols = _scale_single(input, scale_m, meta, reference, mode)
    final = pd.concat([input[input.columns.drop(scaled_cols)], results], axis=1)
...
def scale_and_rate(input, meta, reference=None, boundaries=None, verbose=False):
    ...
    rated = scale(scaled, meta, reference=boundaries, mode='rating', verbose=verbose)
    compound = scale(scaled, meta, reference=boundaries, mode='compound', verbose=verbose)
    ...
    return pd.concat(all_res, axis=1)
