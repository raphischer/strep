...
def chapter3(show):
    ...
    for idx, scale in enumerate(['value', 'index']):
        ...
        traces = create_scatter_graph(plot_data, axis_names, dark_mode=False, display_text=True, marker_width=5, return_traces=True)
    ...
    scatter.update_layout(width=PLOT_WIDTH, height=PLOT_HEIGHT*1.3, margin={'l': 0, 'r': 0, 'b': 0, 't': 18},
                            legend=dict(x=.5, y=0.05, orientation="h", xanchor="center", yanchor="bottom"))
    finalize(scatter, fname, show)
    ...
    fig = go.Figure(layout={'width': PLOT_WIDTH, 'height': PLOT_HEIGHT*1.5, 'margin': {'l': 0, 'r': 0, 'b': 0, 't': 0},
                            'yaxis':{'title': r'$\text{Compound score } S_\Omega(m, C)$'}, 'xaxis': {'range': (-1, len(models)-1)}})
    for env in envs:
    ...
    for p_idx, (host, host_envs) in enumerate(host_envs.items()):
        ...
        fig.update_yaxes(title_text=r'$\text{{H} - Quality score } S_{Q,\Omega}(m, C)$'.replace('{H}', host), range=[0.82, 1.02], row=p_idx+1, col=1)
        x_title = r'$\text{Resource score } S_{R,\Omega}(m, C)$' if p_idx == len(host_envs) - 1 else ''
    ...
    fig.update_layout(width=PLOT_WIDTH, height=PLOT_HEIGHT*2, margin={'l': 0, 'r': 0, 'b': 0, 't': 24},
                      ...
                      xaxis3={'title': r'$\text{Correlation } r(\tilde{\mu}_i, \tilde{\mu}_j, C)$'}, xaxis4={'title': r'$\text{Correlation } r(\tilde{\mu}_i, \tilde{\mu}_j, C)$'}, yaxis3={'visible': False}, yaxis4={'visible': False})
    finalize(fig, fname, show)
    ...
    fig.update_layout(width=PLOT_WIDTH, height=PLOT_HEIGHT, margin={'l': 0, 'r': 0, 'b': 0, 't': 0},
                      yaxis={'title': r'$\text{All correlation } r(\tilde{\mu}_i, \tilde{\mu}_j, C_l)$'})
    finalize(fig, fname, show)
